...
class AltitudeMPC:
    ...
    def setupController(self,next_trajectories, next_controls, cur_time):
        self.opti = ca.Opti()
        # the total thrust
        ...
        for i in range(self.N):
            ...
            exponential_term = ca.exp(-alphaa*(cur_time * self.T - t_o))
    
    # Tính toán các thành phần của objective function
            state_term = ca.mtimes([state_error_, self.Q, state_error_.T])
            control_term = ca.mtimes([control_error_, self.R, control_error_.T])
            exponential_state_term = state_term * exponential_term
    
    # Cộng tất cả các thành phần lại để tính obj
            obj = obj + state_term + control_term + exponential_state_term
...
class PositionMPC:
    ...
    def setupController(self,next_trajectories, next_controls,thrust, cur_time):
        self.opti = ca.Opti()
